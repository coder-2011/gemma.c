...
from gemma4_bench_utils import benchmark_contract, benchmark_environment
...
def env_snapshot():
    ...
    env = benchmark_environment("global_decode_torch_bench")
    env.update({
        "nvidia_smi": try_run([
            "nvidia-smi",
            f"--query-gpu={query}",
...
def main():
    ...
    parser.add_argument("--sample-delay-s", type=float, default=0.0,
                        help="legacy no-op; sleeps between samples are disabled")
    ...
    report = {
        "contract": benchmark_contract(
            name="global_decode_torch_bench",
            measurement="single-token global paged decode attention latency",
            timing="cuda_events_current_pytorch_stream",
            ...
            sample_delay_s_ignored=args.sample_delay_s,
            launch_overhead="excluded_by_cuda_graph_replay",
            timer_overhead=(
