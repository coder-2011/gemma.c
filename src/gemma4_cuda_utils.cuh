...
template <size_t Alignment> inline bool is_aligned_to(const void *ptr) { return (reinterpret_cast<uintptr_t>(ptr) & (Alignment - 1)) == 0; }
...
struct alignas(16) Packed128 {
  ...
  __device__ explicit Packed128(int4 bits) { static_assert(sizeof(bits) == sizeof(payload), "Packed128 size mismatch"); memcpy(payload, &bits, sizeof(bits)); }
...
__device__ inline void gemma4_bf16_pack_accumulate_square(
    ...
    float &sum_sq) {
  const __nv_bfloat162 *pairs = reinterpret_cast<const __nv_bfloat162 *>(values.payload);
#pragma unroll
...
__device__ inline void gemma4_bf16_pack_accumulate_dot(
    ...
    float &sum) {
  const __nv_bfloat162 *x_pairs = reinterpret_cast<const __nv_bfloat162 *>(x_pack.payload);
  const __nv_bfloat162 *w_pairs = reinterpret_cast<const __nv_bfloat162 *>(w_pack.payload);
#pragma unroll
  for (int p = 0; p < kBf16Packed128Pairs; ++p) {
...
__device__ inline Bf16Packed128 gemma4_bf16_pack_add(
  ...
  const __nv_bfloat162 *a_pairs = reinterpret_cast<const __nv_bfloat162 *>(a.payload);
  const __nv_bfloat162 *b_pairs = reinterpret_cast<const __nv_bfloat162 *>(b.payload);
  Bf16Packed128 result;
  __nv_bfloat162 *out_pairs = reinterpret_cast<__nv_bfloat162 *>(result.payload);
#pragma unroll
  for (int p = 0; p < kBf16Packed128Pairs; ++p) {
...
__device__ inline Bf16Packed128 gemma4_bf16_pack_apply_scale(
    const Bf16Packed128 &values,
    float scale) {
  const __nv_bfloat162 *value_pairs = reinterpret_cast<const __nv_bfloat162 *>(values.payload);
  Bf16Packed128 result;
  __nv_bfloat162 *out_pairs = reinterpret_cast<__nv_bfloat162 *>(result.payload);
#pragma unroll
  for (int p = 0; p < kBf16Packed128Pairs; ++p) {
...
__device__ inline Bf16Packed128 gemma4_bf16_pack_apply_scale_weight(
    ...
    float scale) {
  const __nv_bfloat162 *value_pairs = reinterpret_cast<const __nv_bfloat162 *>(values.payload);
  const __nv_bfloat162 *weight_pairs = reinterpret_cast<const __nv_bfloat162 *>(weight.payload);
  Bf16Packed128 result;
  __nv_bfloat162 *out_pairs = reinterpret_cast<__nv_bfloat162 *>(result.payload);
#pragma unroll
  for (int p = 0; p < kBf16Packed128Pairs; ++p) {
