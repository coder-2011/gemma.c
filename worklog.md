...
- Plucked some low hanging fruit: SMEM buffering the data arriving from DRAM, because we load _x_ twice–once for sum of squares, and once for multiplying by _γ_. After a bit of tuning, [[agents fill in performance metrics here.]]

- Did a focused tuning pass on that hidden fused RMSNorm prefill kernel. Added a smaller benchmark that only times `gemma4_residual_add_rmsnorm_bf16` for width 5376 rows > 1, so iteration doesn't sit through cuDNN/frontend setup and unrelated RMSNorm variants. The only real kept changes were minor: `__launch_bounds__(Threads, 2)`, cached input loads for the two residual sources, and no early gamma prefetch. Aggregate best time across rows 4-1024 moved from 0.090220ms to 0.089864ms, so only about 0.39%. Stopped when the next store-cache hint moved less than 0.05%. Not a big win, but the benchmark path is much less annoying and the kernel is a little cleaner.
