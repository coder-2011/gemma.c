...
#ifdef FLASHATTENTION_GEMMA_FWD_ONLY
    TORCH_CHECK(params.is_bf16, "Gemma-only FlashAttention build supports BF16 only.");
    TORCH_CHECK(params.d == 256, "Gemma-only FlashAttention build supports headdim 256 only.");
    TORCH_CHECK(params.num_splits <= 1 && !force_split_kernel,
                "Gemma-only FlashAttention build does not support split-KV forward.");
    if (params.is_causal) {
        run_mha_fwd_<cutlass::bfloat16_t, 256, true>(params, stream);
    } else {
        run_mha_fwd_<cutlass::bfloat16_t, 256, false>(params, stream);
    }
#else
...
#ifdef FLASHATTENTION_GEMMA_FWD_ONLY
    TORCH_CHECK(false, "Gemma-only FlashAttention build does not support backward.");
#else
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
